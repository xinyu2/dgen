...
Cluster::Cluster(GenContext *ctx, int k, double cf, int ct, int clbl) :
	ctx(ctx), dims(0), cmin(0), cmax(0), cmid(0), crad(0), cinvr2(0),
        k(k), cf(cf), ct(ct), clbl(clbl)
{}  // Cluster
...
void Cluster::generate_template(void) {
	...
	for(int iidim = 0; iidim < k; iidim++) {
        ...
        //printf("@ cluster.generate_template, cluster-label is %d\n",clbl);
